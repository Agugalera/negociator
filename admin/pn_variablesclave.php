...
<?php 
	...
	if (isset($_POST['submit'])) {
		...
		$query = "UPDATE dataVarsClave SET puntaje = {$_POST['puntaje']} WHERE usuario = '{$_POST['equipo']}'";
      ...
      <div>
        ...
        <!--id="left-column"-->
					...
					if ($_GET['equipo']) {
						...
						$row = get_single_value ('dataVarsClave', 'usuario', $_GET['equipo']);

						if (mysql_num_rows($row)>0) {

							$results = mysql_fetch_array($row);
							echo "<p>De la empresa ".ucfirst($results['empresa'])." del universo ".$results['universo']."</p>";
							...
							<form method="post" action="pn_variablesclave.php?equipo=<?php echo $_GET['equipo'];?>">
										...
										<option value="">Seleccione...</option>
											...
											<?php 
												...
												for($y = 1; $y<=$puntaje_maximo; $y++) {
													...
													if ($results['puntaje'] == $y) {$selT = "selected='selected'";} else {$selT = "";}
